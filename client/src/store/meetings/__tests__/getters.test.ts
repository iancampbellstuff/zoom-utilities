...
describe('getters', () => {
    ...
    describe('getCurrentUserId', () => {
        it('should get value from state', () => {
            const result = (getters as any).getCurrentUserId(state);
            expect(result).toEqual(state.currentUserId);
        });
    });
    describe('getMeetings', () => {
        it('should get value from state', () => {
            const result = (getters as any).getMeetings(state);
            expect(result).toEqual(state.meetings);
        });
    });
    describe('getSearch', () => {
        it('should get value from state', () => {
            const result = (getters as any).getSearch(state);
            expect(result).toEqual(state.search);
        });
    });
    describe('getUserIds', () => {
        it('should get value from state', () => {
            const result = (getters as any).getUserIds(state);
            expect(result).toEqual(state.userIds);
        });
    });
});
